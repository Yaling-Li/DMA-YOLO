...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args 遍历backbone和head的每一层
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3STR, C3SPP, C3Ghost, ASPP, CBAM, nn.ConvTranspose2d, CoorAttention, CABottleneck, C3CA]:
            ...
            if m in [BottleneckCSP, C3, C3TR, C3STR, C3Ghost, C3CA]:
        ...
        elif m is AdaptConcat:
            # AdaptConcat层则将f中所有的输出自适应累加得到这层的输出channel
            c2 = sum(ch[x] for x in f) #通道数相加
            level = len(f)
            args = [level, *args]  # 输出通道数 哪个维度上相加 每层的维度
